#!/usr/bin/env python3
"""Simple cross-platform timeout wrapper for CI and local scripts."""
...
def terminate_process(proc: subprocess.Popen[bytes], grace: float = 5.0) -> None:
    """Attempt to terminate the subprocess gracefully, then kill if needed."""
    try:
        if os.name == "nt":
            proc.terminate()
        else:
