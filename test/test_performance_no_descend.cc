...
bool iterate_with_raw_libarchive(const std::string &path, std::size_t &entry_count) {
  ...
  if (open_result != ARCHIVE_OK) {
    std::cerr << "Could not open archive with libarchive: " << archive_error_string(handle.get()) << std::endl;
    return false;
  }
  ...
  while (true) {
    ...
    if (header_result == ARCHIVE_WARN) {
      ...
      std::cerr << "libarchive error while reading header: " << archive_error_string(handle.get()) << std::endl;
      return false;
    }
    ...
    if (skip_result == ARCHIVE_WARN) {
      std::cerr << "libarchive warning while skipping data: " << archive_error_string(handle.get()) << std::endl;
    } else if (skip_result != ARCHIVE_OK) {
      std::cerr << "libarchive error while skipping data: " << archive_error_string(handle.get()) << std::endl;
      return false;
    }
  }
...
std::optional<BenchmarkResult> run_benchmark(const std::string &label, int iterations, const std::function<bool(std::size_t &)> &operation) {
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    if (result.entries == 0) {
      ...
      std::cerr << "Entry count mismatch in " << label << " (expected " << result.entries << ", got " << entries << ")" << std::endl;
      return std::nullopt;
    }

    result.durations.push_back(seconds);

    std::cout << "  Run " << std::setw(2) << (i + 1) << ": " << format_seconds(seconds) << " s (entries=" << entries << ")" << std::endl;
  }

  std::cout << "  Average: " << format_seconds(compute_average(result.durations)) << " s" << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  const std::string archive_path = (argc >= 2) ? argv[1] : "test_data/test_perf.zip";
  ...
  const auto raw_result = run_benchmark("libarchive (raw)", kIterations, [&](std::size_t &entries) { return iterate_with_raw_libarchive(archive_path, entries); });
  ...
  const auto traverser_result = run_benchmark("archive_r Traverser", kIterations, [&](std::size_t &entries) { return iterate_with_traverser(archive_path, entries); });
  ...
  std::cout << "  libarchive (raw) avg: " << format_seconds(raw_avg) << " s" << std::endl;
  std::cout << "  archive_r Traverser avg: " << format_seconds(traverser_avg) << " s" << std::endl;
